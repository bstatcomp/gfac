#' Train a negative binomial Gaussian process factor analysis with copula
#'
#' This function trains a negative binomial Gaussian process factor analysis with copula with a locally periodic kernel in Stan.
#'
#' @export
...
#' @param init_list list. List of initialization parameters for the model. We recommend using the posterior means of gNBGPFA model.
#' @return A list.
...
train_gNBGPFAC <- function (X, ts, gp, nfac, nit, nchain, period_length,
                            prior_r2, prior_r3, prior_phi_type = 1, prior_phi,
                            transform_t, tseed = NULL, init_list = NULL,  ...) {
  ...
  if (!is.null(init_list)) {
    ...
    samps    <- rstan::sampling(stanmodels$gNBGPFAC_LP, stan_data, seed = tseed,
                                chains = nchain,
                                iter = nit,
                                init = il,
                                ...)
