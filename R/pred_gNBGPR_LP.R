#' Evaluate a negative binomial Gaussian process regression
#'
#' This function evaluates a negative binomial Gaussian process regression on test data. Additionally,
#' it provides generated quantities for the test data.
#'
#' @export
#' @param mod list. Output of \code{train_gNBGPR_LP}.
...
pred_gNBGPR_LP <- function (mod, X_test, ts_test, gp_test, ts_train, gp_train, transform_t, period_length) {
  # Generate quantities
  ...
  for (i in 1:nit) {
    ...
    for (j in 1:length(sc)) {
      sc[j] <- prod(dnbinom(as.numeric(X_test[j, ]), mu = as.numeric(X_pred[j, ]), size = phis[i, ]))
    }
    LS_mat[ ,i]  <- sc
  }
