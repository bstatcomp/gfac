...
F_mat <- as.matrix(data.frame(D1 = c(F1 * 4, F4 * 4), D2 = c(F2 * 6, F5 * 6)))
...
# Poisson likelihood -----------------------------------------------------------
sds <- sqrt(apply(X, 2, mean)) * 2

## random covariance
set.seed(2)
A <- matrix(rnorm(99), nrow = 9)
B <- A %*% t(A)
cor_mat <- cov2cor(B)
...
# ggsave("./data_clean/plots/toy_F3.pdf", width = 16, height = 14)
...
save(data_out, file = "./data/toy_G.RData")
