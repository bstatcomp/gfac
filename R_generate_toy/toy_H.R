...
F2 <- draw_samples(x, 1, seed = 10, lp_kernel, alpha2 = 0.5, rho2 = 5, rho3 = 7 * 2)
F4 <- draw_samples(x, 1, seed = 42, se_kernel, alpha1 = 0.5, rho1 = 3 * 2)
F5 <- draw_samples(x, 1, seed = 23, lp_kernel, alpha2 = 0.5, rho2 = 4, rho3 = 8 * 2)
...
F_mat <- as.matrix(data.frame(D1 = c(F1 * 2, F4 * 2), D2 = c(F2 * 2, F5 * 2)))
...
for (i in 1:nrow(X_out)) {
  X_out[i, ] <- abs(round(mvrnorm(n = 1, mu = X[i, ], Sigma = my_sigma)))
...
# ggsave("./data_clean/plots/toy_G.pdf", width = 16, height = 14)
...
save(data_out, file = "./data/toy_H.RData")
