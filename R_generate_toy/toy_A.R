...
F3 <- draw_samples(x, 1, seed = 22, p_kernel, alpha2 = 0.5, rho2 = 6 * 2)
...
F6 <- draw_samples(x, 1, seed = 11, p_kernel, alpha2 = 0.5, rho2 = 5 * 2)
...
F_mat <- as.matrix(data.frame(D1 = c(F1, F4), D2 = c(F2, F5), D3 = c(F3*2, F6*2)))
...
save(data_out, file = "./data/toy_A.RData")
